...
public class TextController : MonoBehaviour {
	...
	private enum States {
			newBiz, open, late, closed, closed_confirm, view_promo_1, view_promo_2, view_promo_3, view_promo_4, accept_promo_1,
			accept_promo_2, accept_promo_3, accept_promo_4, read_landlord_email, call_landlord, pay_landlord, work, open5, open8
			};
	...
	void Update () {
		...
		}	else if (myState == States.view_promo_1) 			{promo_1();
		}	else if (myState == States.view_promo_2) 			{promo_2();
	...
	void work() {	//create story and scenario
		text.text = "You turn on the mixer " +
			" " +
			" \n\n" +
			"Press '' to , '' to , '' to  " +
			"and '' to : ";
			if (Input.GetKeyDown(KeyCode.O))		{myState = States.open;	//show work option
		}	else if (Input.GetKeyDown(KeyCode.O))	{myState = States.open;	//show work option
		}	else if (Input.GetKeyDown(KeyCode.O))	{myState = States.open;	//show work option
		}	else if (Input.GetKeyDown(KeyCode.O))	{myState = States.open;	//show work option
		}	
	}
	...
	void promo_1 () { //add accept promotion 1 or 2,
		text.text = "The first promotion is $100 for 200 pounds of strawberries. You can make 10,000 " +
			...
			"Press 'F2' to View Promotion 2, L to read the email from your landlord and W to Work " +
	...
	void promo_2 () { //add accept promotion 1 or 2
		text.text = "The second promotion is $200 for 375 pounds of oranges. You can make 7,500 " +
			...
			"Press 'F1' to View Promotion 1, L to read the email from your landlord , and W to Work " +
			"and start making popsicles immediately: ";
			if 		(Input.GetKeyDown(KeyCode.W))	{myState = States.newBiz;	//if you choose 'N' give option to read the inventory that needs to be restocked on the board.
	...
	void read_landlord_email () { //add accept promotion 1 or 2
			...
			"IN THIS TOWN! \n\n" +
			"Press U to Call the Landlord, 'F1' to View Promotion 1, 'F2' to View Promotion 2 and W to Work " +
			"and start making popsicles immediately: ";
			if 		(Input.GetKeyDown(KeyCode.W))	{myState = States.newBiz;	//if you choose 'N' give option to read the inventory that needs to be restocked on the board.
		}	else if (Input.GetKeyDown(KeyCode.F1))	{myState = States.view_promo_1;	//show promo 1
		}	else if (Input.GetKeyDown(KeyCode.F2))	{myState = States.view_promo_2;	//show promo 2
